...
newtype ElabM a = ElabM { unElabM :: WriterT (DList Constraint) TcM a }
    deriving
        ...
        (body', mty') <- runElabM $ do
            ...
            mty' <- case mty of
                ...
                Just ty -> do
                    ...
                    checkEqual (exprRange body') C0 Universe (exprType body') (exprTerm ty')
                    return (Just ty')
            return (body', mty')
...
typeCheckExpr ctx names = \case
    Syntax.Var l n -> do
        ...
        tt <- case nameKind n of
            ...
            Unbound -> throwDiagnostic l ("unbound name: " ++ Text.unpack name)
...
typeCheckPi :: Range -> Ctx -> UTerm -> UTerm -> ElabM UTerm
